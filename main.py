...
def get_quant_analysis():
    print(
        "🔍 대형주 300선 대상 기술 지표 및 거래량/수급 턴어라운드 정밀 스캔 가동..."
    )
    ...
    for symbol in tickers:
        try:
            ...
            volume_ratio_5d = (
                (current_volume / avg_volume_5d) * 100
                if avg_volume_5d > 0
                else 0
            )
            ...
            mad = tp.rolling(window=20).apply(
            ...
            # 4. 야후 거래량 기반 최근 5거래일 추정 자금 순유입량 계산
    ...
    final_df = final_df.dropna(
        subset=["RSI", "MFI", "CCI", "거래량비율(5일평균대비)", "종합점수"]
    )
    return final_df
...
if __name__ == "__main__":
    ...
    if not report.empty:
        ...
        report.to_csv(
            "daily_quant_report.csv", index=False, encoding="utf-8-sig"
        )
        print(
            f"📊 [성공] 총 {len(report)}개 종목 분석 완료! 신규 거래량 지표가 포함된 수급 리포트가 정상 생성되었습니다."
        )
